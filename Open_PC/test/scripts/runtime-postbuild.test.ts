...
describe("runtime postbuild static assets", () => {
  ...
  it("ignores tracked extension packages deleted from a vendored working tree", async () => {
    const rootDir = createTempDir("openclaw-runtime-postbuild-");
    const packagePath = path.join(rootDir, "extensions", "removed", "package.json");
    await fs.mkdir(path.dirname(packagePath), { recursive: true });
    await fs.writeFile(packagePath, JSON.stringify({ name: "@openclaw/removed" }), "utf8");
    execFileSync("git", ["init", "-q"], { cwd: rootDir });
    execFileSync("git", ["add", "extensions/removed/package.json"], { cwd: rootDir });
