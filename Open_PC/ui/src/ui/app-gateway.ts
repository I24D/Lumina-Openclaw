...
function formatGatewayCloseMessage(code: number, reason: string): string {
  if (isLuminaDesktopShell() && code === 1006) {
    return "Reconectando gateway local. Lumina seguira abierta y volvera automaticamente.";
  }
  return `disconnected (${code}): ${reason || "no reason"}`;
}
  ...
  const client = new GatewayBrowserClient({
    ...
    onClose: ({ code, reason, error }) => {
      ...
      if (code !== 1012) {
        ...
        host.lastError =
          shutdownHost.pendingShutdownMessage ?? formatGatewayCloseMessage(code, reason);
      } else {
