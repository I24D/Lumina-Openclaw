...
export async function interruptChatForModelSwitch(host: ChatHost) {
  const hadActiveRun = Boolean(host.chatRunId) || host.chatSending;
  host.chatQueue = [];
  if (host.connected && hadActiveRun) {
    await abortChatRun(host as unknown as ChatState);
  }
  host.chatQueue = [];
  ...
  const streamState = host as ChatHost & { chatStreamStartedAt?: number | null };
  streamState.chatStreamStartedAt = null;
...
function shouldQueueLocalSlashCommand(name: string): boolean {
  return !["stop", "model", "focus", "export-session", "steer", "redirect", "new"].includes(name);
}
