...
let providerCustomApiKey = "";
let providerCustomPanelOpen = false;
let providerCustomProviderId = "openai";
let providerCustomBaseUrl = "https://api.openai.com/v1";
let providerCustomModelId = "gpt-5.4-pro";
let providerCustomModelName = "GPT-5.4 Pro";
let providerCustomApiFormat = "openai-completions";
...
function mergeProviderModelEntry(
  existingModels: unknown,
  nextModel: Record<string, unknown>,
): Record<string, unknown>[] {
  const list = Array.isArray(existingModels)
    ? existingModels.filter(
        (entry): entry is Record<string, unknown> =>
          Boolean(entry) && typeof entry === "object" && !Array.isArray(entry),
      )
    : [];
  const modelId = typeof nextModel.id === "string" ? nextModel.id.trim().toLowerCase() : "";
  const filtered = modelId
    ...
    : list;
  return [nextModel, ...filtered];
}

async function switchProviderModel(state: AppViewState, nextModelRef: string) {
  const currentModel =
    (state.configForm?.agents as { defaults?: { model?: { primary?: string } } } | null | undefined)
      ?.defaults?.model?.primary ?? null;
  if (currentModel === nextModelRef) {
    return;
  }
  await interruptChatForModelSwitch(state);
  let persistWarning: string | null = null;
  if (isLuminaDesktopShell() && window.__LUMINA__?.savePreferredModel) {
    try {
      await window.__LUMINA__.savePreferredModel(nextModelRef);
      await loadConfig(state);
    } catch (err) {
      persistWarning = `Model changed, but failed to save the desktop default: ${String(err)}`;
    }
  } else {
    updateConfigFormValue(state, ["agents", "defaults", "model", "primary"], nextModelRef);
    if (!(await saveConfig(state))) {
      return;
    }
  }
  ...
  try {
    ...
    state.chatModelOverrides = {
      ...state.chatModelOverrides,
      [state.sessionKey]: createChatModelOverride(nextModelRef),
    };
    ...
    state.lastError = `Model saved as default, but it could not be applied to the active session: ${String(err)}`;
  }
}
...
export function renderApp(state: AppViewState) {
  ...
  if (shouldRequireLuminaAuth() && !isLuminaAuthenticated()) {
    ...
    const formState = state as AppViewState & LuminaFormState;
    formState.luminaAuthMode ??= "login";
    formState.luminaEmail ??= "";
    formState.luminaPassword ??= "";
    formState.luminaLoading ??= false;
    formState.luminaError ??= null;
    return html`${renderLuminaAuthGate(
      Object.assign(formState, {
        requestUpdate: requestHostUpdate ?? (() => undefined),
      }),
    )}`;
  }

  // Browser use requires a gateway session; desktop can render while its local gateway starts.
  if (!state.connected && !isLuminaDesktopShell()) {
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${state.tab === "provider"
          ? renderProviderSetup({
              ...
              activeModel:
                (
                  state.configForm?.agents as
                    | { defaults?: { model?: { primary?: string } } }
                    | null
                    | undefined
                )?.defaults?.model?.primary ?? null,
              customPanelOpen: providerCustomPanelOpen,
              customProviderId: providerCustomProviderId,
              customApiKey: providerCustomApiKey,
              customBaseUrl: providerCustomBaseUrl,
              ...
              onCustomFieldChange: (field, value) => {
                if (field === "providerId") providerCustomProviderId = value;
                else if (field === "apiKey") providerCustomApiKey = value;
                else if (field === "baseUrl") providerCustomBaseUrl = value;
                else if (field === "modelId") providerCustomModelId = value;
                else if (field === "modelName") providerCustomModelName = value;
                else if (field === "apiFormat") providerCustomApiFormat = value;
                notifyLazyViewChanged();
              },
              onSaveCustomProvider: async () => {
                const providerId = providerCustomProviderId.trim() || "custom";
                const modelId = providerCustomModelId.trim();
                const modelName = providerCustomModelName.trim() || modelId;
                const baseUrl = providerCustomBaseUrl.trim();
                const apiKey = providerCustomApiKey.trim();
                const api = providerCustomApiFormat.trim() || "openai-completions";
                if (!modelId || !apiKey || !baseUrl) {
                  return;
                }
                const configRoot = asConfigRecord(state.configForm ?? state.configSnapshot?.config);
                const modelsRoot = asConfigRecord(configRoot.models);
                const providersRoot = asConfigRecord(modelsRoot.providers);
                const existingProvider = asConfigRecord(providersRoot[providerId]);
                const nextModel = {
                  id: modelId,
                  name: modelName,
                  contextWindow: Number(existingProvider.contextWindow) || 16000,
                  maxTokens: Number(existingProvider.maxTokens) || 4096,
                  input: ["text"],
                  cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
                  reasoning: /gpt-5|opus|sonnet|reasoner|r1/iu.test(modelId),
                };
                updateConfigFormValue(state, ["models", "providers", providerId], {
                  ...existingProvider,
                  baseUrl,
                  api,
                  apiKey,
                  models: mergeProviderModelEntry(existingProvider.models, nextModel),
                });
                await switchProviderModel(state, `${providerId}/${modelId}`);
                providerCustomPanelOpen = false;
                providerCustomApiKey = "";
                notifyLazyViewChanged();
              },
            })
          : nothing}
