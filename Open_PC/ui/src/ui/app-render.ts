...
export function renderApp(state: AppViewState) {
  ...
  if (!isLuminaAuthenticated()) {
    ...
    const ls = state as AppViewState & LuminaFormState;
    if (ls.luminaAuthMode === undefined) ls.luminaAuthMode = "login";
    if (ls.luminaEmail    === undefined) ls.luminaEmail    = "";
    if (ls.luminaPassword === undefined) ls.luminaPassword = "";
    if (ls.luminaLoading  === undefined) ls.luminaLoading  = false;
    if (ls.luminaError    === undefined) ls.luminaError    = null;

    const update = requestHostUpdate ?? (() => {});

    return html`${renderLuminaAuthGate({
      luminaAuthMode:    ls.luminaAuthMode,
      luminaEmail:       ls.luminaEmail,
      luminaPassword:    ls.luminaPassword,
      luminaLoading:     ls.luminaLoading,
      luminaError:       ls.luminaError,
      requestUpdate: update,
    })}`;
  }

  // Gate 2: require successful gateway connection before showing the dashboard.
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">LUMINA</span>
                      </span>
                    `}
              </div>
              <button
