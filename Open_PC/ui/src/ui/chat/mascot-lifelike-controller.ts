...
export class MascotLifelikeController {
  ...
  private loop(): void {
    ...
    // ── Antenna sway ── always-on slow oscillation; speaking amps it up.
    const speakingBoost = this.state === "speaking" ? 1.6 : this.state === "thinking" ? 1.3 : 1.0;
    const antenna = Math.sin((elapsed / ANTENNA_PERIOD_MS) * Math.PI * 2 + this.phaseAntenna)
      * ANTENNA_MAX_DEG * speakingBoost;
    ...
    if (now >= this.nextSaccadeAtMs) {
      ...
      this.nextSaccadeAtMs = now + SACCADE_INTERVAL_MIN_MS + Math.random() * span;
    }
    ...
    if (this.state === "speaking") {
      mouthTarget = this.readAudioAmplitude();
      if (mouthTarget === null) {
        mouthTarget = this.syntheticEnvelope(elapsed);
      }
    }
    ...
    s.setProperty("--lumina-head-x", `${headX.toFixed(2)}px`);
