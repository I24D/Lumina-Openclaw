...
import { createChatSessionsLoadOverrides, interruptChatForModelSwitch } from "../app-chat.ts";
...
async function switchChatModel(state: AppViewState, nextModel: string): Promise<boolean> {
  ...
  switchPromise = (async () => {
    try {
      ...
      if (nextModel && typeof window !== "undefined") {
        try {
          await window.__LUMINA__?.savePreferredModel?.(nextModel);
        } catch (err) {
