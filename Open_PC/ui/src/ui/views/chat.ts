...
function resolveLiveToolActivity(toolMessages: unknown[]): LiveToolActivity[] {
  return toolMessages
    .map((message) => {
      const cards = extractToolCards(message);
      const call = cards.find((card) => card.args !== undefined || card.inputText !== undefined);
      if (!call) {
        return null;
      }
      return {
        name: call.name,
        label: resolveToolDisplay({ name: call.name, args: call.args }).label,
        complete: call.outputText !== undefined,
      };
    })
    .filter((activity): activity is LiveToolActivity => Boolean(activity))
    .slice(-4);
}

function renderLiveActivity(props: ChatProps, isBusy: boolean) {
  if (!isBusy && !props.canAbort) {
    return nothing;
  }
  const tools = resolveLiveToolActivity(props.toolMessages);
  const hasResponseText = Boolean(props.stream?.trim());
  const summary = hasResponseText
    ? "Redactando respuesta en tiempo real"
    : tools.length > 0
  ...
  return html`
    <section
      class="chat-live-activity"
      role="status"
      aria-live="polite"
      aria-label="Actividad en vivo"
    >
      <div class="chat-live-activity__header">
        <span class="chat-live-activity__pulse" aria-hidden="true"></span>
        <span class="chat-live-activity__title">Actividad en vivo</span>
        <span class="chat-live-activity__summary">${summary}</span>
      </div>
      ${tools.length > 0
        ? html`
            <div class="chat-live-activity__steps">
              ${tools.map(
                (activity) => html`
                  <span
                    class="chat-live-activity__step ${activity.complete
                      ? "chat-live-activity__step--complete"
                      : "chat-live-activity__step--active"}"
                    title=${activity.name}
                  >
                    ${activity.complete ? icons.check : icons.loader} ${activity.label}
                  </span>
                `,
              )}
            </div>
  ...
  const thread = html`
    ...
    >
      <div class="chat-thread-inner">
        ...
        ${renderLiveActivity(props, isBusy)} ${renderRealtimeTalkConversation(props)}
      </div>
    </div>
  `;
