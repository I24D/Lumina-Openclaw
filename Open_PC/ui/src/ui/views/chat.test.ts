...
describe("chat loading skeleton", () => {
  it("renders live activity while waiting for the initial response", () => {
    const container = renderChatView({ sending: true, canAbort: true });

    const activity = container.querySelector(".chat-live-activity");
    expect(activity).not.toBeNull();
    expect(activity?.textContent).toContain("Actividad en vivo");
    expect(activity?.textContent).toContain("Preparando respuesta");
  });

  it("surfaces live tool activity without expanding tool output", () => {
    const container = renderChatView({
      canAbort: true,
      toolMessages: [
        {
    ...
    const activity = container.querySelector(".chat-live-activity");
    expect(activity?.textContent).toContain("Trabajando con herramientas");
    expect(activity?.querySelector(".chat-live-activity__step--active")).not.toBeNull();
  });

  it("reports that response text is streaming while content is rendered", () => {
    const container = renderChatView({
      sending: true,
      canAbort: true,
      stream: "Respuesta parcial",
    });

    const activity = container.querySelector(".chat-live-activity");
    expect(activity?.textContent).toContain("Redactando respuesta en tiempo real");
  });
