...
describe("applyPluginAutoEnable core", () => {
  ...
  it("skips auto-enable when desktop startup suppression is enabled", () => {
    const config = {
      channels: { slack: { botToken: "x" } },
      plugins: { allow: ["slack"], entries: { slack: { enabled: true } } },
    };
    const suppressedEnv = { ...env, OPENCLAW_SKIP_PLUGIN_AUTO_ENABLE: "1" };
    ...
    const result = applyPluginAutoEnable({ config, env: suppressedEnv });
