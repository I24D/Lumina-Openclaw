...
import {
  type GatewayRemoteBrainLoopResult,
  type RemoteBrainMessage,
...
function extractUsageFromResult(result: unknown): Usage {
  const directUsage = (result as { usage?: unknown } | null)?.usage;
  if (directUsage && typeof directUsage === "object") {
    const openAiUsage = directUsage as {
    ...
    if (
      ...
      return {
        input_tokens: Math.max(0, openAiUsage.prompt_tokens ?? 0),
        output_tokens: Math.max(0, openAiUsage.completion_tokens ?? 0),
        total_tokens: Math.max(
          0,
          openAiUsage.total_tokens ??
            (openAiUsage.prompt_tokens ?? 0) + (openAiUsage.completion_tokens ?? 0),
        ),
      };
    }
  }
...
function buildRemoteBrainMessagesFromResponsesInput(params: {
  input: string | ItemParam[];
  extraSystemPrompt: string;
}): RemoteBrainMessage[] {
  const messages: RemoteBrainMessage[] = [];
  ...
  for (const item of params.input) {
    if (item.type === "message") {
      const content = typeof item.content === "string" ? item.content : extractTextContent(item.content);
      if (!content.trim()) {
        continue;
      }
      const role =
...
function buildRemoteBrainResponsesResult(
  remoteBrain: Extract<GatewayRemoteBrainLoopResult, { enabled: true; ok: true }>,
) {
  return {
    payloads: remoteBrain.reply ? [{ text: remoteBrain.reply }] : undefined,
    usage: remoteBrain.usage,
    meta: {
      stopReason: remoteBrain.finishReason,
      pendingToolCalls: remoteBrain.pendingToolCalls?.map((toolCall) => ({
        id: toolCall.id,
...
export async function handleOpenResponsesHttpRequest(
  ...
  const streamParams =
    typeof payload.max_output_tokens === "number"
      ...
      : undefined;
  let remoteBrainResult:
    | ReturnType<typeof buildRemoteBrainResponsesResult>
    | null = null;
  if (images.length === 0 && fileContexts.length === 0) {
    const remoteBrain = await runGatewayRemoteBrainLoop({
      ...
      senderIsOwner,
      userId: user,
      messages: buildRemoteBrainMessagesFromResponsesInput({
        input: payload.input,
        extraSystemPrompt,
      }),
    ...
    if (remoteBrain.enabled) {
      if (!remoteBrain.ok) {
        logWarn(`openresponses remote brain failed: ${remoteBrain.error}`);
        const response = createResponseResource({
          id: responseId,
          model,
          status: "failed",
          output: [],
          error: { code: "api_error", message: remoteBrain.error },
        });
        rememberResponseSession();
        sendJson(res, 502, response);
        return true;
      }
      remoteBrainResult = buildRemoteBrainResponsesResult(remoteBrain);
    }
  }
