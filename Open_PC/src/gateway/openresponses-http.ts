...
function extractRemoteBrainTextContent(item: ItemParam & { type: "message" }): string {
...
function buildRemoteBrainMessagesFromResponsesInput(params: {
  ...
  if (params.extraSystemPrompt) {
    messages.push({ role: "system", content: params.extraSystemPrompt });
  }
  ...
  for (const item of params.input) {
    if (item.type === "message") {
      const content = extractRemoteBrainTextContent(item);
      if (!content.trim()) {
        continue;
      }
      const role =
...
export async function handleOpenResponsesHttpRequest(
  ...
  const streamParams =
    streamMaxTokens !== undefined || streamTemperature !== undefined || streamTopP !== undefined
      ...
      : undefined;
  let remoteBrainResult: ReturnType<typeof buildRemoteBrainResponsesResult> | null = null;
  if (images.length === 0 && fileContexts.length === 0) {
    const remoteBrain = await runGatewayRemoteBrainLoop({
      ...
      runtimeContext: { surface: "openresponses" },
