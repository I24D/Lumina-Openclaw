...
export async function startGatewayServer(
  ...
  const desktopFastStart = process.env.OPENCLAW_DESKTOP_FAST_START === "1";
      ...
      loadGatewayStartupPostAttachModule().then(
        ({ startGatewayPostAttachRuntime, stopPostReadySidecarsAfterCloseStarted }) =>
          startGatewayPostAttachRuntime({
            ...
            startupTrace,
            deferSidecars: opts.deferStartupSidecars === true || desktopFastStart,
            providerAuthPrewarm: { enabled: !desktopFastStart, getConfig: getRuntimeConfig },
          }),
      ),
    ));
