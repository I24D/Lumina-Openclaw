...
import { type RemoteBrainMessage, type RemoteBrainToolCall, runGatewayRemoteBrainLoop } from "./remote-brain.js";
...
function writeAssistantToolCallChunk(
  res: ServerResponse,
  params: { runId: string; model: string; toolCall: RemoteBrainToolCall; index: number },
) {
  writeSse(res, {
    id: params.runId,
    object: "chat.completion.chunk",
...
function normalizeAssistantToolCall(message: Record<string, unknown>, index: number) {
    ...
    .map((toolCall) => {
      ...
      const id =
        typeof toolCall.id === "string" && toolCall.id.trim()
          ...
          : `call_${index + 1}`;
      const name =
        typeof functionShape.name === "string" && functionShape.name.trim()
          ? functionShape.name.trim()
          : undefined;
      if (!name) {
        return null;
      }
...
function buildRemoteBrainMessagesFromOpenAiMessages(messagesUnknown: unknown): RemoteBrainMessage[] {
  ...
  return messages
    .map((message, index) => {
      ...
      if (role === "system" || role === "user" || role === "assistant") {
        return {
          ...
          ...(role === "assistant"
            ? { tool_calls: normalizeAssistantToolCall(message as Record<string, unknown>, index) }
            : {}),
        } satisfies RemoteBrainMessage;
      }
      if (role === "tool") {
        const toolCallId =
          typeof (message as Record<string, unknown>).tool_call_id === "string"
            ? ((message as Record<string, unknown>).tool_call_id as string).trim()
            : undefined;
        if (!toolCallId) {
          return null;
        }
    ...
    .filter((message): message is RemoteBrainMessage => message !== null);
}

function toOpenAiCompatToolCalls(toolCalls: RemoteBrainToolCall[]) {
...
export async function handleOpenAiHttpRequest(
  ...
  const remoteBrainMessages = buildRemoteBrainMessagesFromOpenAiMessages(payload.messages);
  if (images.length === 0) {
    const remoteBrain = await runGatewayRemoteBrainLoop({
      cfg: loadConfig(),
      sessionKey,
      requestId: runId,
      messageChannel,
      ...
      senderIsOwner,
      userId: user,
      messages: remoteBrainMessages,
      runtimeContext: {
        surface: "openai_chat_completions",
      },
    });
    if (remoteBrain.enabled) {
      if (!remoteBrain.ok) {
        logWarn(`openai-compat remote brain failed: ${remoteBrain.error}`);
        sendJson(res, 502, {
          error: { message: remoteBrain.error, type: "api_error" },
        });
        return true;
      }

      const finishReason =
        remoteBrain.finishReason === "tool_calls" ? "tool_calls" : "stop";
      const compatToolCalls = toOpenAiCompatToolCalls(remoteBrain.toolCalls);
      if (!stream) {
        sendJson(res, 200, {
          id: runId,
          ...
          choices: [
            {
              ...
              message: {
                role: "assistant",
                content: remoteBrain.reply,
                ...(compatToolCalls.length > 0 ? { tool_calls: compatToolCalls } : {}),
              },
      ...
      compatToolCalls.forEach((toolCall, index) => {
      ...
      writeAssistantFinishChunk(res, { runId, model, finishReason });
      writeDone(res);
      res.end();
      return true;
    }
