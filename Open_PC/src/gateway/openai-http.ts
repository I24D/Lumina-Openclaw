...
function normalizeRemoteBrainToolCalls(
...
function buildRemoteBrainMessagesFromOpenAiMessages(
  messagesUnknown: unknown,
): RemoteBrainMessage[] {
  return asMessages(messagesUnknown)
    .map((message, index) => {
      ...
      if (role === "system" || role === "user" || role === "assistant") {
        return {
          ...
          ...(role === "assistant"
            ? { tool_calls: normalizeRemoteBrainToolCalls(record, index) }
            : {}),
...
export async function handleOpenAiHttpRequest(
  ...
  if (images.length === 0) {
    const remoteBrain = await runGatewayRemoteBrainLoop({
      ...
      senderIsOwner,
      userId: user,
      messages: buildRemoteBrainMessagesFromOpenAiMessages(payload.messages),
      clientTools: resolvedClientTools,
      runtimeContext: { surface: "openai_chat_completions" },
    ...
    if (remoteBrain.enabled) {
      ...
      const finishReason = remoteBrain.finishReason === "tool_calls" ? "tool_calls" : "stop";
      const toolCalls = remoteBrain.toolCalls.map((call) => ({
        id: call.id,
      ...
      writeAssistantToolCallsIncrementalChunks(res, { runId, model, toolCalls });
