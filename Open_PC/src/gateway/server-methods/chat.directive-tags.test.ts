...
import type { GatewayRemoteBrainLoopResult } from "../remote-brain.js";
...
const mockState = vi.hoisted(() => ({
  ...
  remoteBrainResult: null as GatewayRemoteBrainLoopResult | null,
...
vi.mock("../remote-brain.js", async () => {
  const original = await vi.importActual<typeof import("../remote-brain.js")>("../remote-brain.js");
  return {
    ...original,
    runGatewayRemoteBrainLoop: vi.fn(
      async (...args: Parameters<typeof original.runGatewayRemoteBrainLoop>) =>
        mockState.remoteBrainResult ?? original.runGatewayRemoteBrainLoop(...args),
    ),
  };
});
...
describe("chat directive tag stripping for non-streaming final payloads", () => {
  afterEach(() => {
    ...
    mockState.remoteBrainResult = null;
  ...
  it("streams remote brain replies through chat.send delta events before final", async () => {
    createTranscriptFixture("openclaw-chat-send-remote-stream-");
    const reply =
      "Remote work is now visible as it progresses in the desktop chat. " +
      "This answer is long enough to arrive in multiple display updates before completion.";
    mockState.remoteBrainResult = {
      enabled: true,
      ok: true,
      reply,
    ...
    await runNonStreamingChatSend({
      context,
      respond,
      idempotencyKey: "idem-remote-stream",
      waitFor: "none",
    });

    await waitForAssertion(() => {
      expect(lastBroadcastPayload(context)?.state).toBe("final");
    });

    const chatPayloads = (context.broadcast as unknown as ReturnType<typeof vi.fn>).mock.calls.map(
      (call) => call[1] as Record<string, any>,
    );
    const deltas = chatPayloads.filter((payload) => payload.state === "delta");
    expect(deltas.length).toBeGreaterThan(1);
    expect(deltas.map((payload) => payload.deltaText).join("")).toBe(reply);
    expect(extractFirstTextBlock(deltas.at(-1))).toBe(reply);
    expect(chatPayloads.map((payload) => payload.state)).toEqual([
      ...deltas.map(() => "delta"),
      "final",
    ]);
    expect(extractFirstTextBlock(chatPayloads.at(-1))).toBe(reply);
  });
