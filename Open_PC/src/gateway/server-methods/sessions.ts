...
export const sessionsHandlers: GatewayRequestHandlers = {
  ...
  "sessions.patch": async ({ req, params, respond, context, client, isWebchatConnect }) => {
    ...
    if (Object.prototype.hasOwnProperty.call(p, "model")) {
      const interruptResult = await interruptSessionRunIfActive({
        req,
        context,
        client,
        isWebchatConnect,
        requestedKey: key,
        canonicalKey: target.canonicalKey ?? key,
      });
      if (interruptResult.error) {
        respond(false, undefined, interruptResult.error);
        return;
