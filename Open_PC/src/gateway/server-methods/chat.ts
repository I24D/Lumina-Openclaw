...
function splitRemoteBrainChatStreamText(text: string): string[] {
  const targetLength = Math.max(32, Math.ceil(text.length / 60));
  const chunks: string[] = [];
  let cursor = 0;
  while (cursor < text.length) {
    let end = Math.min(text.length, cursor + targetLength);
    if (end < text.length) {
      const candidate = text.slice(cursor, end);
      const breakAt = Math.max(candidate.lastIndexOf(" "), candidate.lastIndexOf("\n"));
      if (breakAt >= Math.floor(targetLength / 2)) {
...
// Remote brains currently return complete turns; publish gateway deltas so the desktop chat
// presents their visible response progressively, while live tool events remain real-time.
async function broadcastRemoteBrainChatDeltas(params: {
  ...
  const text = sanitizeAssistantDisplayText(params.text);
  if (!text) {
    return;
  }
  const chunks = splitRemoteBrainChatStreamText(text);
  let renderedText = "";
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const payload = {
      runId: params.runId,
      sessionKey: params.sessionKey,
      seq: nextChatSeq({ agentRunSeq: params.context.agentRunSeq }, params.runId),
      state: "delta" as const,
      deltaText: chunks[index],
      message: {
        role: "assistant",
    ...
    try {
      ...
      if (parsedImages.length === 0) {
        ...
        const remoteBrain = await runGatewayRemoteBrainLoop({
          ...
          userId: clientInfo?.id ?? sessionKey,
          messages: [
            {
              role: "user",
              content: stampedMessage,
            },
          ],
          runtimeContext: {
            surface: "chat.send",
        ...
        if (remoteBrain.enabled) {
          ...
          if (!remoteBrain.ok) {
            throw new Error(remoteBrain.error);
          }

          let message: Record<string, unknown> | undefined;
          if (remoteBrain.reply.trim()) {
            const { storePath: latestStorePath, entry: latestEntry } = loadSessionEntry(sessionKey);
            ...
            if (appended.ok) {
              ...
              context.logGateway.warn(
                `webchat remote brain transcript append failed: ${appended.error ?? "unknown error"}`,
              );
              message = {
                role: "assistant",
          ...
          await broadcastRemoteBrainChatDeltas({
            context,
            runId: clientRunId,
            sessionKey,
            text: remoteBrain.reply,
            signal: activeRunAbort.controller.signal,
          });
