...
export async function createModelSelectionState(params: {
  ...
  const configuredProviderIds = Object.keys(cfg.models?.providers ?? {}).map(normalizeProviderId);
  const useStaticLuminaDesktopModelNormalization =
    process.env.OPENCLAW_DESKTOP_FAST_START === "1" &&
    normalizeProviderId(defaultProvider) === "lumina" &&
    configuredProviderIds.length > 0 &&
    configuredProviderIds.every((providerId) => providerId === "lumina");
  const visibilityPolicyOptions = useStaticLuminaDesktopModelNormalization
    ? {
        allowManifestNormalization: false as const,
        allowPluginNormalization: false as const,
        manifestPlugins: [],
      }
    : {};
  ...
  const configuredModelCatalog = buildConfiguredModelCatalog({
    cfg,
    ...(useStaticLuminaDesktopModelNormalization ? { manifestPlugins: [] } : {}),
  });
