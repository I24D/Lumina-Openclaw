...
afterEach(() => {
  delete process.env.OPENCLAW_DESKTOP_FAST_START;
...
describe("createModelSelectionState catalog loading", () => {
  ...
  it("skips dynamic plugin model normalization for the Lumina desktop local provider", async () => {
    ...
    const cfg = {
      ...
      models: {
        providers: {
          lumina: {
            models: [makeConfiguredModel({ id: "gpt-5.5" })],
          },
        },
      },
    } as OpenClawConfig;
    ...
    expect(state.allowedModelKeys.has("lumina/gpt-5.5")).toBe(true);
