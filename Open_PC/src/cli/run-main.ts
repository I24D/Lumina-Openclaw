...
export async function runCli(argv: string[] = process.argv) {
  ...
  // Lumina desktop already owns the local provider proxy process during startup.
  // Avoid an additional CLI-managed proxy bootstrap and config read in that host.
  const shouldUseCliEnvProxy =
    !isTruthyEnvValue(process.env.OPENCLAW_DESKTOP_FAST_START) &&
    !isHelpOrVersionInvocation &&
    shouldStartProxyForCli(normalizedArgv);
