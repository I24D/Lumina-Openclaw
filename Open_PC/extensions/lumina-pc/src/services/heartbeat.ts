...
export function createHeartbeatService(config: HeartbeatConfig): OpenClawPluginService {
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (snap.cpu_pct > 90) {
        ctx.logger.warn(`[lumina-heartbeat] HIGH CPU: ${snap.cpu_pct}% — I24D should investigate.`);
      }
      ...
      ctx.logger.error(`[lumina-heartbeat] tick error: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
