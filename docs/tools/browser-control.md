...
- Managed Playwright profiles move the pointer to `click-coords` targets with a short, bounded minimum-jerk trajectory before clicking. The final point remains the exact requested CSS-pixel coordinate. This improves visible cursor continuity; it is not an anti-bot or CAPTCHA bypass.
