  ...
  it("separates portable facts from WhatsApp transport callbacks", async () => {
    ...
    expect(prepared.inbound).toMatchObject({
      ...
      conversation: {
        ...
        // Lumina labels conversations by group subject (or sender name for
        // direct chats) instead of the raw JID, so notifications and session
        // lists are readable. Upstream still labels with the id.
        label: "Boundary Room",
      },
      reply: {
        replyToId: "quoted-1",
    ...
    expect(prepared.ctxPayload).toMatchObject({
      // Same Lumina labelling rule as conversation.label above.
      ConversationLabel: "Boundary Room",
      GroupSubject: "Boundary Room",
    });
...
describe("whatsapp inbound dispatch", () => {
  ...
  it("removes inline provider reasoning before WhatsApp delivery", async () => {
    const deliverReply = vi.fn(async () => acceptedDeliveryResult());
