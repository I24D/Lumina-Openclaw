...
export type LoopAction =
  ...
  | (LoopActionCommon & {
      kind: "browser_smart_click";
      query: string;
      role?: string;
      exact?: boolean;
    })
  | (LoopActionCommon & {
      kind: "browser_smart_type";
      query: string;
      text: string;
      pressEnter?: boolean;
    })
...
export function buildUserPrompt(p: ThinkParams): string {
  ...
  const fg = obs.foregroundProcess
    ? `${obs.foregroundProcess}${obs.foregroundTitle ? ` — "${obs.foregroundTitle}"` : ""}`
    : "unknown";
  const interactables = (obs.interactables ?? [])
    ...
    .join("\n");
  const windows = (obs.windowTitles ?? [])
    .slice(0, 8)
    .map((t) => `  - ${t}`)
    .join("\n");
  const history =
