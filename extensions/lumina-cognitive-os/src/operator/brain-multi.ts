...
class BrainCache {
  ...
  private computeKey(params: ThinkParams): string {
    const data = JSON.stringify({
      goal: params.goal,
      screenshotHash: params.screenshotPath,
  ...
  const openai: BrainChoice | null = (() => {
    ...
    return {
      ...
      model:
        ...
        "gpt-4o-mini",
      baseUrl:
        envFirst(["OPENAI_PC_OPERATOR_BASE_URL", "OPENAI_BASE_URL"], envOpts) ??
...
async function callAnthropic(
  ...
  const raw = (await fetchJson(
    fetchImpl,
    joinUrl(choice.baseUrl ?? "https://api.anthropic.com", "/v1/messages"),
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
    ...
    case "gemini": {
      ...
      if (!raced.ok) {
        throw new TransientBrainError(
          504,
          `gemini call exceeded perProviderTimeoutMs=${timeoutMs}ms`,
        );
      }
