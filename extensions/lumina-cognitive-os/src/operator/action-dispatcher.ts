...
async function callTool(
  ...
  try {
    ...
    const verifiedFromBrowser =
      typeof details.verified === "boolean" ? (details.verified as boolean) : null;
...
async function dispatchKeyPress(
  ...
  const resp = await bridge.post<{ ok?: boolean; allowed?: boolean; error?: string }>(
    "/input_control",
    {
      action,
      keys,
...
async function dispatchOpenApplication(
  bridge: BridgeClient,
  application: string,
): Promise<DispatchResult> {
