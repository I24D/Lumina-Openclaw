  ...
  return {
    ...
    parameters: Type.Object({
      goal: Type.String({
        ...
        maxLength: 500,
        description:
          "Meta en lenguaje natural. Sé concreto: 'abre Chrome y ve a youtube.com' es mejor que 'pon música'.",
      }),
      mode: Type.Optional(
        Type.Union([Type.Literal("simulate"), Type.Literal("production")], {
          default: "production",
          description:
            "simulate = decide pero NO dispatcha (preview). production = ejecuta de verdad.",
        }),
      ),
      maxIterations: Type.Optional(
        Type.Integer({
          ...
          maximum: 20,
          default: 8,
          description: "Hard ceiling on loop iterations.",
        }),
      ),
      interStepDelayMs: Type.Optional(
        Type.Integer({
          ...
          maximum: 2_000,
          default: 250,
          description: "Pausa entre steps para que la UI se asiente.",
        }),
      ),
      preferBrowser: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Forzar el uso de browser_* tools (cuando la foreground no es browser pero el goal es web).",
        }),
      ),
      ...
      brainModel: Type.Optional(
        Type.String({
          ...
          maxLength: 120,
          description:
            "Modelo opcional para el brain interno, por ejemplo gemma4:31b o gemini-2.5-flash.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      const raw = rawParams as {
        ...
        brainProvider?: "auto" | "gemini" | "openai" | "anthropic" | "ollama";
        brainModel?: string;
      };
      const params = raw as {
        goal: string;
...
export function createPcDoStatusTool(engine: PcOperatorEngine): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { runId: string };
...
export function createPcDoSkillHealthTool(tracker: SkillHealthTracker): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { flaggedOnly?: boolean };
...
export function createPcDoSkillResetTool(tracker: SkillHealthTracker): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { skillName: string };
...
export function createPcDoCostSummaryTool(meter: CostMeter): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { windowSeconds?: number; limit?: number };
      const params = raw as { windowSeconds?: number; limit?: number };
      return jsonResult(
        meter.summary({ windowSeconds: params.windowSeconds, limit: params.limit }),
      );
    },
  };
}
