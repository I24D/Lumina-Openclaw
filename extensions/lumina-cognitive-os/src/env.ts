...
export function getLuminaEnvVar(name: string, opts: LoadEnvOptions = {}): string | undefined {
...
export function requireLuminaEnvVar(name: string, opts: LoadEnvOptions = {}): string {
  ...
  if (v === undefined) {
    throw new Error(`Missing env var ${name}. Add it to ${opts.envPath ?? DEFAULT_ENV_PATH}.`);
  }
  return v;
}
