...
async function callSidecar(
  ...
  timeoutMs: number,
): Promise<{ ok: boolean; data?: SidecarResponse; error?: string }> {
  return runPythonSidecarJson<SidecarResponse>("browser_drive", [], {
    timeoutMs,
    ...
    parameters: Type.Object({
      ...
      role: Type.Optional(
        Type.String({
          maxLength: 30,
          description:
            "Opcional ARIA role hint (button, link, menuitem, tab, checkbox, radio, option, textbox, searchbox).",
        }),
      ),
      exact: Type.Optional(
        Type.Boolean({
          default: false,
          description: "Si true, match exacto del name (caso-sensitive).",
        }),
      ),
...
export function createBrowserDomObserveTool(opts: BrowserSmartConfig): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { limit?: number; profile?: string; headless?: boolean };
      if (!opts.enabled) {
        return jsonResult({ ok: false, error: "browser driver is disabled." });
      }
