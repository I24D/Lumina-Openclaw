...
function makeDeps(over: Partial<SmartClickDeps> = {}): {
  deps: SmartClickDeps;
  calls: FetchCall[];
  setResponses: (r: Array<Partial<Response> & { json?: () => Promise<unknown> }>) => void;
} {
...
function setUiaFind(
  ...
  >,
  process: { name: string; pid: number | null; className: string } | null = {
    name: "chrome.exe",
    pid: 1234,
...
describe("lumina_smart_click", () => {
  it("resolves UIA, dispatches click, verifies via screenshot_diff", async () => {
    ...
    const clickPayload = payload<{ action: string; x: number; y: number; allowedApps: string[] }>(
      calls[1]!,
    );
  ...
  it("aborts when processName hint doesn't match foreground process", async () => {
    const { deps, calls } = makeDeps();
    setUiaFind([{ name: "Play", score: 0.95, center: { x: 10, y: 10 } }], {
      name: "notepad.exe",
      pid: 99,
      className: "Notepad",
    });
...
describe("lumina_smart_type", () => {
  it("clicks to focus then types into a resolved Edit field", async () => {
    const { deps, calls, setResponses } = makeDeps();
    setUiaFind([{ name: "Search", controlType: "Edit", score: 0.88, center: { x: 400, y: 50 } }]);
    setResponses([
      { json: async () => ({ ok: true, path: "C:/tmp/pre.png" }) }, // pre screenshot
  ...
  it("dryRun returns picked candidate without dispatching", async () => {
    ...
    const tool = createSmartTypeTool(deps);
    const res = await tool.execute(FAKE_TOOL_CALL_ID, {
      query: "Search",
      text: "hi",
      dryRun: true,
    });
