...
async function dispatchType(deps: SmartClickDeps, text: string): Promise<DispatchResult> {
  const body = await bridgeJson<DispatchResult>(
    deps,
    "/input_control",
    {
...
const ClickParams = Type.Object({
  ...
  controlType: Type.Optional(
    Type.String({
      maxLength: 40,
      description:
        "Filtro UIA opcional: Button, Edit, Hyperlink, MenuItem, CheckBox, ComboBox, ...",
    }),
  ),
  ...
  clicks: Type.Optional(
    Type.Integer({
      ...
      maximum: 3,
      default: 1,
      description: "1=click, 2=doble-click, 3=triple.",
    }),
  ),
...
const TypeParams = Type.Object({
  ...
  pressEnter: Type.Optional(
    Type.Boolean({
      default: false,
      description: "Si true, manda ENTER al final (submit del campo).",
    }),
  ),
  verify: Type.Optional(
    Type.Union(
...
export function createSmartClickTool(deps: SmartClickDeps): AnyAgentTool {
  return {
    ...
    async execute(_id, raw) {
      ...
      const filtered = filterByProcess(
        resolved.matches ?? [],
        params.processName,
        resolved.process,
      );
      ...
      const preScreenshot =
        verifyKind === "uia" || verifyKind === "none" ? null : await captureScreenshot(deps);
    ...
    async execute(_id, raw) {
      ...
      if (!picked) {
        return jsonResult({
          ...
          alternatives: filtered.slice(0, 3),
          hint: "No encontré un campo claro. Considera controlType='Edit' o pasa el processName de la app.",
        });
      }
      if (dryRun) {
        return jsonResult({
