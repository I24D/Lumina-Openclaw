...
async function pickCenter(
  ...
  | {
      ...
      match: UiaMatch;
      process: { pid: number | null; name: string; className: string } | undefined;
    }
...
export function createPcObserveTool(deps: PcToolsDeps): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      maxInteractables: Type.Optional(
        Type.Integer({
          ...
          maximum: 200,
          default: 40,
          description: "Top-N UIA nodes a devolver.",
        }),
      ),
      ...
      includeUia: Type.Optional(
        Type.Boolean({
          default: true,
          description: "Leer árbol UIA (interactables + foreground app).",
        }),
      ),
      includeWindows: Type.Optional(
        Type.Boolean({
          default: true,
          description: "Listar ventanas visibles via Bridge /window_control list.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      const maxInteractables =
        typeof params.maxInteractables === "number" ? params.maxInteractables : 40;
      const want = {
      ...
      const [screenshotResp, uiaResp, windowsResp] = await Promise.all([
        ...
        want.windows
          ? c.post<{
              ok?: boolean;
              windows?: Array<{ title?: string; pid?: number; process?: string; bbox?: unknown }>;
            }>("/window_control", { action: "list" }, 4_000)
          : Promise.resolve(null),
      ]);
  ...
  return {
    ...
    parameters: Type.Object({
      direction: Type.Optional(
        Type.Union(
          SCROLL_DIRECTIONS.map((d) => Type.Literal(d)),
          { default: "down" },
        ),
      ),
      amount: Type.Optional(
        Type.Integer({
          ...
          maximum: 50,
          default: 3,
          description: "Notches del wheel (1 ≈ 3 líneas).",
        }),
      ),
      query: Type.Optional(
        Type.String({
          maxLength: 200,
          description: "Opcional: hover encima de este elemento antes de scroll.",
        }),
      ),
    ...
    async execute(_id, rawParams) {
      ...
      if (params.query) {
        ...
        if (
          params.processName &&
          picked.process &&
          picked.process.name?.toLowerCase() !== params.processName.toLowerCase()
        ) {
          return jsonResult({
            ok: false,
    ...
    parameters: Type.Object({
      ...
      steps: Type.Optional(
        Type.Integer({
          ...
          maximum: 200,
          default: 24,
          description: "Cuántos pasos intermedios para suavizar.",
        }),
      ),
      stepDelayMs: Type.Optional(Type.Integer({ minimum: 0, maximum: 200, default: 8 })),
    ...
    async execute(_id, rawParams) {
      ...
      if (params.fromQuery) {
        ...
        if (!picked.ok) {
          return jsonResult({
            ok: false,
            error: `from:${picked.error}`,
            alternatives: picked.matches ?? [],
          });
        }
        from = picked.point;
