  ...
  return {
    ...
    parameters: Type.Object({
      ...
      outDir: Type.Optional(
        Type.String({ maxLength: 512, description: "Output directory for screencast videos." }),
      ),
      maxDurationMs: Type.Optional(
        Type.Number({
          ...
          maximum: 600_000,
          default: 300000,
          description: "Max screencast duration in ms (5 min default).",
        }),
      ),
      scale: Type.Optional(
        Type.Number({
          ...
          maximum: 1,
          default: 0.5,
          description: "Screencast scale (0.1-1.0). 0.5 = 50% resolution for smaller files.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      const r = await runPythonSidecarJson<{ ok: boolean; [k: string]: unknown }>(
        ...
        [],
        {
          timeoutMs: Math.max(15_000, (params.timeoutMs ?? 5_000) + 10_000),
