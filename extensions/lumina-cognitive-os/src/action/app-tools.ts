...
function client(deps: AppToolsDeps): BridgeClient {
  return (
    deps.clientOverride ??
    createBridgeClient({ bridgeUrl: deps.bridgeUrl, fetchImpl: deps.fetchImpl })
  );
}
  ...
  return {
    ...
    parameters: Type.Object({
      filter: Type.Optional(
        Type.String({
          maxLength: 80,
          description: "Substring opcional. 'office' → Word, Excel, etc.",
        }),
      ),
      limit: Type.Optional(Type.Integer({ minimum: 10, maximum: 500, default: 200 })),
    }),
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { filter?: string; limit?: number };
