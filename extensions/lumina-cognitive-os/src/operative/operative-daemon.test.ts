...
import { loadOperativeRules, renderSuggestion, ruleMatches } from "./operative-rules.js";
...
describe("loadOperativeRules — validation", () => {
  ...
  it("rejects rule without id", () => {
    const rulesPath = writeRules([
      { trigger: { kind: "battery.low" }, suggestion: { title: "T", message: "M" } },
    ]);
    const set = loadOperativeRules(rulesPath);
  ...
  it("leaves unknown placeholders untouched", () => {
    const set = loadOperativeRules(
      writeRules([
        {
          id: "x",
          trigger: { kind: "cpu.high" },
          suggestion: { title: "CPU {pct}% on {host}", message: "ok" },
        },
      ]),
    );
