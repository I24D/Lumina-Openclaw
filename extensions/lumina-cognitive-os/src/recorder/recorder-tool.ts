  ...
  return {
    ...
    parameters: Type.Object({
      label: Type.Optional(
        Type.String({
          maxLength: 200,
          description: "Human-readable label for the demo ('organize downloads').",
        }),
      ),
      mode: Type.Optional(
        Type.Union([Type.Literal("events"), Type.Literal("screencast")], {
          default: "events",
          description:
            "`events`: one frame per real event. `screencast`: also tick a frame every 1/fpsHint seconds.",
        }),
      ),
      captureUia: Type.Optional(
        Type.Boolean({
          default: true,
          description: "Snapshot the Windows UIA tree on each event.",
        }),
      ),
      fpsHint: Type.Optional(
        Type.Number({
          ...
          maximum: 30,
          default: 5,
          description: "Only used when mode=screencast.",
        }),
      ),
      sessionId: Type.Optional(
        Type.String({
          maxLength: 64,
          description: "Optional fixed session id; auto-generated otherwise.",
        }),
      ),
    }),
...
export function createRecorderPauseTool(deps: RecorderToolDeps): AnyAgentTool {
  return {
    ...
    description:
      "Pauses event capture without closing the session. Use to type a password without recording it.",
    parameters: Type.Object({}),
    async execute() {
...
export function createRecorderGetTool(deps: RecorderToolDeps): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      ...
      const events = deps.recorder.store.readEvents(id, {
        offset: p.offset ?? 0,
        limit: p.limit ?? 100,
      });
      return jsonResult({
