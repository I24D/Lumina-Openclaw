...
export class RecorderProcess {
  ...
  private pendingStart:
    | ((
        res: { ok: true; sessionId: string; sessionDir: string } | { ok: false; error: string },
      ) => void)
    | null = null;
  private pendingStop:
    | ((
        res:
          | { ok: true; stats: { events: number; durationMs: number; sessionDir: string } }
  ...
  async start(
    params: StartParams = {},
  ): Promise<{ ok: true; sessionId: string; sessionDir: string } | { ok: false; error: string }> {
