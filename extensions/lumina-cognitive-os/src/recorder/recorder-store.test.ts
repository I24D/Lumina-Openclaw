...
describe("RecorderStore — read", () => {
  ...
  it("summarize fills derived fields", () => {
    ...
    writeSession(store, "test", [
      { idx: 1, atMs: 0, kind: "session.start", screenshot: "screenshots/000001.png" },
      {
        ...
        pos: { x: 10, y: 20 },
        button: "left",
        screenshot: "screenshots/000002.png",
      },
    ]);
  ...
  it("list sorts by startedAtISO descending", () => {
    const store = new RecorderStore(tmpDir);
    writeSession(store, "older", [{ idx: 1, atMs: 0, kind: "x" }], {
      startedAtISO: "2026-06-26T09:00:00.000Z",
    });
    writeSession(store, "newer", [{ idx: 1, atMs: 0, kind: "x" }], {
      startedAtISO: "2026-06-28T09:00:00.000Z",
    });
...
describe("redactSecretsInText", () => {
  ...
  it("redacts sk- api keys", () => {
    expect(redactSecretsInText("sk-abc12345abc12345abc12345abc12345", p)).toMatch(
      /REDACTED:apikey/,
    );
  });
