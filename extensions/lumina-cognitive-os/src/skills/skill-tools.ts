...
export function createSkillListTool(loader: SkillLoader): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      includeErrors: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Include skills that failed to load (with the load error), useful for debugging.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      const params = rawParams as { includeErrors?: boolean };
    ...
    async execute(_id, rawParams) {
      ...
      if (!skill) {
        return jsonResult({
          ...
          hint:
            "Call lumina_skill_list to see what's installed, or drop a SKILL.md into " +
            loader.dirForDebug(),
        });
      }
      const instructions =
        skill.instructions.length > MAX_DESCRIBE_INSTRUCTIONS
  ...
  return {
    ...
    parameters: Type.Object({
      ...
      relPath: Type.String({
        ...
        maxLength: 256,
        description:
          "Path inside the skill folder, e.g. 'scripts/extract.py' or 'references/REFERENCE.md'.",
      }),
      ...
      binary: Type.Optional(
        Type.Boolean({
          default: false,
          description: "If true, return base64; otherwise utf-8 text.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
...
export function createSkillRunTool(loader: SkillLoader, log: ActionLogStore | null): AnyAgentTool {
  return {
