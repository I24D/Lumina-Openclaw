  ...
  private loadOne(folder: string, skillDir: string, skillFile: string): Skill | { error: string } {
    ...
    if (parsed.data.version !== undefined) {
      ...
      if (!isValidSemVer(parsed.data.version)) {
        return {
          error: `frontmatter \`version\` (${parsed.data.version}) must be valid SemVer (e.g., "1.0.0", "2.1.3-beta")`,
        };
      }
  ...
  readAsset(
    ...
    try {
      ...
      return {
        ...
        bytes: stat.size,
        content:
          encoding === "base64" ? buf.toString("base64") : buf.toString(encoding as BufferEncoding),
      };
    } catch (e) {
