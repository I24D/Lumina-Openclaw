...
export function createAwarenessSnapshotTool(poller: AwarenessPoller): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      ...
      const params = rawParams as { fresh?: boolean };
      const fresh = params.fresh === true;
      const snap = fresh
        ? await readEnvironmentSnapshot()
        : (poller.current() ?? (await readEnvironmentSnapshot()));
...
export function createAwarenessSubscribeTool(bus: AwarenessEventBus): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      limit: Type.Optional(Type.Number({ minimum: 1, maximum: 128, default: 32 })),
    }),
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
