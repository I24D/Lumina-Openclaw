...
import { classifyForegroundWindow, classifyWindow, strategyOrder } from "./window-classify.js";
  ...
  return {
    ...
    parameters: Type.Object({
      processName: Type.Optional(
        Type.String({ description: "ej. chrome.exe, code.exe, explorer.exe" }),
      ),
      className: Type.Optional(Type.String({ description: "Win32 class, ej. Chrome_WidgetWin_1" })),
      elevated: Type.Optional(
        Type.Boolean({ description: "La ventana corre elevada (integridad alta)." }),
      ),
    }),
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const raw = rawParams as { processName?: string; className?: string; elevated?: boolean };
