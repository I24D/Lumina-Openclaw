  ...
  return {
    ...
    parameters: Type.Object({
      query: Type.String({
        ...
        maxLength: 200,
        description:
          "Natural-language label of the element to find (e.g. 'Guardar', 'campo de búsqueda').",
      }),
      controlType: Type.Optional(
        Type.String({
          maxLength: 40,
          description:
            "Optional UIA ControlType filter: Button, Edit, Hyperlink, MenuItem, CheckBox, ComboBox, ...",
        }),
      ),
      pid: Type.Optional(
        Type.Number({ description: "Optional PID; defaults to the foreground window." }),
      ),
      maxMatches: Type.Optional(Type.Number({ minimum: 1, maximum: 20, default: 5 })),
      maxDepth: Type.Optional(Type.Number({ minimum: 1, maximum: 12, default: 8 })),
      maxNodes: Type.Optional(Type.Number({ minimum: 50, maximum: 2000, default: 600 })),
    }),
