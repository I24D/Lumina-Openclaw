  ...
  return {
    ...
    parameters: Type.Object({
      ...
      includeOcr: Type.Optional(
        Type.Boolean({
          default: true,
          description: "Run the OCR pass for text regions. Disable for speed.",
        }),
      ),
      maxElements: Type.Optional(Type.Number({ minimum: 5, maximum: 300, default: 60 })),
      setOfMarks: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Also return a numbered description + index suitable for LLM Set-of-Marks grounding.",
        }),
      ),
      minConfidence: Type.Optional(Type.Number({ minimum: 0, maximum: 1, default: 0 })),
    }),
  ...
  return {
    ...
    async execute() {
      const r = await runPythonSidecarJson<{
        ...
        device: string;
        modelDir: string;
        haveYoloWeights: boolean;
        haveFlorenceLocal: boolean;
        haveTorch: boolean;
