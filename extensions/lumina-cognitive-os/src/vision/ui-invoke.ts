  ...
  return {
    ...
    parameters: Type.Object({
      ...
      action: Type.Optional(
        Type.String({
          default: "invoke",
          description: "invoke | click | set_value | toggle | select | focus",
        }),
      ),
      value: Type.Optional(Type.String({ description: "Text to write when action is set_value." })),
      nameMatch: Type.Optional(
        Type.String({ default: "contains", description: "contains | exact (how name is matched)" }),
      ),
      pid: Type.Optional(
        Type.Number({
          description: "Target window process ID. Defaults to the foreground window.",
        }),
      ),
    }),
