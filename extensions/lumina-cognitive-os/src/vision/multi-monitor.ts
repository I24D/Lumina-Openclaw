...
export function createMultiMonitorTool(config: { screenshotDir?: string } = {}): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      ...
      maxMonitors: Type.Optional(Type.Number({ minimum: 1, maximum: 8, default: 4 })),
    }),
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const params = rawParams as { includeBase64?: boolean; maxMonitors?: number };
      const platform = detectPlatform();
      if (!canRunWindowsHostTools()) {
        return jsonResult({
          ok: false,
          error:
            `multimonitor capture needs Windows or WSL (current platform: ${platform}). ` +
