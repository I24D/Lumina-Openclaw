...
export type PreconditionProcessRunning = {
  readonly type: "process_running";
...
function evaluatePrecondition(
  ...
  if (precondition.type === "process_running") {
    ...
    return running
      ? { skip: true, reason: `process '${precondition.name}' already running` }
      : { skip: false };
  }
  if (precondition.type === "window_title_contains") {
    ...
    return hit
      ? { skip: true, reason: `window matching '${precondition.needle}' already visible` }
      : { skip: false };
  }
  if (precondition.type === "window_missing") {
    ...
    return hit
      ? { skip: false }
      : { skip: true, reason: `no window matching '${precondition.needle}'` };
  }
  return { skip: false };
}
