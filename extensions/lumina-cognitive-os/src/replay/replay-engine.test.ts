...
function liveProvider(
  over: Partial<Awaited<ReturnType<LiveContextProvider>>> = {},
): LiveContextProvider {
  return async () => ({
    screenshotPath: null,
    uiaNodes: null,
...
describe("ReplayEngine — basic lifecycle", () => {
  ...
  it("returns error for missing session", async () => {
    ...
    await expect(engine.run({ sessionId: "missing", mode: "simulate" })).rejects.toThrow(
      /not found/,
    );
  });
});
  ...
  it("clicks the matched element's center", async () => {
    ...
    const ctx = {
      ...
      live: {
        ...
        uiaNodes: [
          {
            automationId: "openBtn",
            name: "Open",
            bbox: { x: 10, y: 20, w: 60, h: 30 },
            center: { x: 40, y: 35 },
          },
          {
            automationId: "saveBtn",
            name: "Save",
            bbox: { x: 700, y: 400, w: 100, h: 40 },
            center: { x: 750, y: 420 },
          },
        ],
        windows: [],
...
describe("vision_grounded — without OmniParser client", () => {
  it("skips when client not configured", async () => {
    const { visionGroundedStrategy, configureOmniParserClient } =
      await import("./strategies/vision-grounded.js");
