...
export function createReplayRunTool(engine: ReplayEngine): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      ...
      verifyEachStep: Type.Optional(
        Type.Boolean({ description: "Default: true when mode=production." }),
      ),
      ...
      confirm: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Required true when mode='production' to protect against accidental dispatch.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      if (mode === "production" && !p.confirm) {
        return jsonResult({
          ...
          hint:
            "Production replay actually clicks/types. Show the user the recording summary and " +
