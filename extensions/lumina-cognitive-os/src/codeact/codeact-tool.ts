  ...
  return {
    ...
    parameters: Type.Object({
      ...
      sessionId: Type.Optional(
        Type.String({
          maxLength: 64,
          description: "Optional fixed session id; otherwise auto-generated.",
        }),
      ),
    }),
    async execute(_id, rawParams) {
      ...
      const p = rawParams as { goal: string; maxIterations?: number; sessionId?: string };
      try {
...
export function createCodeActStepTool(engine: CodeActEngine): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const p = rawParams as { sessionId: string; code: string; timeoutMs?: number };
      try {
...
export function createCodeActStatusTool(engine: CodeActEngine): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const p = rawParams as { sessionId: string };
...
export function createCodeActEndTool(engine: CodeActEngine): AnyAgentTool {
  return {
    ...
    async execute(_id, rawParams) {
      // Narrowed once against this tool's schema; the tool runtime
      // validates the payload before execute() is ever called.
      const p = rawParams as { sessionId: string; status?: "done" | "aborted" };
