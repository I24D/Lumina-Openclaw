...
export class CodeActEngine {
  ...
  constructor(opts: CodeActOptions) {
    this.risk = opts.risk;
    this.log = opts.log;
    this.workspaceRoot = path.resolve(opts.workspaceRoot ?? "c:/I24D_WhatsApp/codeact-workspace");
  ...
  async step(params: {
    ...
    if (!sidecarResult.ok && !sidecarResult.stdout.trim()) {
      throw new Error(
        `code_executor sidecar failed: ${sidecarResult.error ?? sidecarResult.stderr}`,
      );
    }
    let parsed: {
      ok: boolean;
    ...
    this.log?.append({
      ...
      detail:
        finalValue !== undefined ? `FINAL emitted` : `exit ${step.exit} in ${step.durationMs}ms`,
      source: "codeact-engine",
...
function scanStdout(stdout: string): { finalValue: unknown | undefined; observations: unknown[] } {
  ...
  for (const line of stdout.split(/\r?\n/)) {
    ...
    if (line.startsWith(OBS_PREFIX)) {
      ...
      try {
        observations.push(JSON.parse(payload));
      } catch {
