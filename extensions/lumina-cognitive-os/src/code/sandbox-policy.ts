...
const HARD_DENY: ReadonlyArray<{ id: string; needle: RegExp; reason: string }> = [
  {
    id: "shutdown",
    needle: /\bshutdown(\.exe)?\b|\bRestart-Computer\b|\bStop-Computer\b/i,
    reason: "Shutdown / restart command.",
  },
  {
    id: "format",
    needle: /\bformat\s+[a-z]:|\bClear-Disk\b|\bFormat-Volume\b/i,
    reason: "Disk format command.",
  },
  {
    id: "rm-rf",
    needle: /\brm\s+-rf\s+\/|\brm\s+-fr\s+\/|\bRemove-Item\b[^|]*-Recurse[^|]*-Force[^|]*[\\/]/i,
    reason: "Recursive force delete on root.",
  },
  {
    id: "del-system",
    needle: /\bdel\s+\/[fsq][\s/]*[a-z]:\\?(Windows|Program)/i,
    reason: "del on a system path.",
  },
  {
    id: "reg-delete-hklm",
    needle: /\breg\s+delete\b.*HKLM|\bRemove-ItemProperty\b.*HKLM/i,
    reason: "Registry HKLM delete.",
  },
...
export function preflightCheck(params: {
  ...
  if (!SUPPORTED_LANGUAGES.includes(lang)) {
    return {
      ok: false,
      reason: `language '${params.language}' is not supported. Allowed: ${SUPPORTED_LANGUAGES.join(", ")}`,
    };
  }
  ...
  if (params.policy.cwdAllow.length > 0) {
    ...
    if (!allowed) {
      return {
        ok: false,
        reason: `cwd '${params.cwd}' is outside the allowlist. Allowed roots: ${params.policy.cwdAllow.join(", ")}`,
      };
    }
  }
  ...
  const requested =
    typeof params.timeoutMs === "number" ? params.timeoutMs : params.policy.defaultTimeoutMs;
