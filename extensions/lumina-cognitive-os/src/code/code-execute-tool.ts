  ...
  return {
    ...
    parameters: Type.Object({
      ...
      code: Type.String({
        ...
        maxLength: 64_000,
        description:
          "Source code to execute. Will be passed via -c / -e / -Command (NOT through a shell).",
      }),
      cwd: Type.Optional(
        Type.String({
          maxLength: 1024,
          description:
            "Absolute working directory. Must be inside the cwd allowlist. Defaults to %USERPROFILE%.",
        }),
      ),
      timeoutMs: Type.Optional(
        Type.Number({
          minimum: 100,
          maximum: policy.maxTimeoutMs,
          default: policy.defaultTimeoutMs,
        }),
      ),
      env: Type.Optional(
      ...
      confirm: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Set to true only on a SECOND call after the user explicitly confirmed a HIGH_RISK action.",
        }),
      ),
    }),
