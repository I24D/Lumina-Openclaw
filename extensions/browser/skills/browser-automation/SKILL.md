...
3. Read before you click:
   ...
   - Use the cheapest observation that answers the current question: bounded `text` for prose, an efficient/query-filtered snapshot for semantic discovery, and labels or a screenshot only when position or appearance matters. Do not send repeated full-page images when the document and target have not changed.
4. Act narrowly:
   ...
   - Prefer refs over coordinates. Coordinate clicks are a visual fallback; managed Playwright profiles smooth their pointer travel, but they still depend on the viewport staying stable and the target remaining unobscured.
