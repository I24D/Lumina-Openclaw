...
export class ProfilePage extends OpenClawLightDomElement {
  ...
  private renderAvatar(avatarUrl: string | null, name: string) {
    if (avatarUrl) {
      return html`<img class="profile-hero__avatar-image" src=${avatarUrl} alt=${name} />`;
    }
    return html`<img
      class="profile-hero__avatar-image profile-hero__avatar-favicon"
      src=${controlUiPublicAssetPath("favicon.svg", this.context.basePath)}
      alt=${name}
    />`;
  }

  private renderHero(insights: ProfileInsights | null) {
    const { agentId, name, avatarUrl } = this.featuredAgent();
    ...
    return renderSettingsGroup(html`
      <section class="profile-hero">
        <div class="profile-hero__avatar">${this.renderAvatar(avatarUrl, name)}</div>
