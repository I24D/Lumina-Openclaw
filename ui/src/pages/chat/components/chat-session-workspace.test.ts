...
describe("openSessionWorkspaceFile", () => {
  it.each(["report.pdf", "proposal.docx", "inventory.xlsx", "deck.pptx", "notes.rtf"])(
    "routes %s through the document preview",
    (path) => {
      expect(isDocumentPreviewPath(path)).toBe(true);
    },
  );

  it("opens Office files from the existing sidebar without decoding them as text", async () => {
    const handleOpenSidebar = vi.fn();
    const getFile = vi.fn();
    const listFiles = vi.fn().mockResolvedValue({
      sessionKey: "agent:main:current",
      root: "C:\\Users\\dal_n\\.openclaw\\workspace",
      files: [
        {
          path: "Inventario_Tecnologia.xlsx",
          name: "Inventario_Tecnologia.xlsx",
          kind: "modified",
          missing: false,
          size: 42_000,
        },
    ...
    openSessionWorkspaceFile(state, { path: "Inventario_Tecnologia.xlsx" });
    ...
    expect(handleOpenSidebar.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "Inventario_Tecnologia.xlsx",
      path: "Inventario_Tecnologia.xlsx",
      source: "C:\\Users\\dal_n\\.openclaw\\workspace\\Inventario_Tecnologia.xlsx",
      size: 42_000,
    });
  });
