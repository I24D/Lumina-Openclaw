...
export function isDocumentPreviewPath(filePath: string): boolean {
  ...
  return DOCUMENT_PREVIEW_EXTENSIONS.has(extension);
}

function isAbsoluteWorkspacePath(filePath: string): boolean {
...
function openFile(
  ...
  if (isDocumentPreviewPath(requestPath)) {
    openWorkspaceItem(
      state,
      workspace,
      `file:${path}`,
      (request) => {
        const currentList = workspace.list;
        if (currentList?.sessionKey === request.sessionKey && currentList.root) {
          return Promise.resolve(currentList);
        }
        return state.sessions.listFiles(request.sessionKey, { agentId: request.agentId });
      },
      (result) => {
        const root = result.root ?? workspace.list?.root ?? null;
        const source = isAbsoluteWorkspacePath(requestPath)
          ? requestPath
          : root
            ? workspaceBrowserFilePath(root, requestPath)
            : null;
        if (!source) {
          return null;
        }
        const normalizedPath = path.replaceAll("\\", "/").toLocaleLowerCase();
        const entry = result.files.find((file) =>
          [file.path, file.workspacePath]
            .filter((candidate): candidate is string => Boolean(candidate))
            .some(
              (candidate) => candidate.replaceAll("\\", "/").toLocaleLowerCase() === normalizedPath,
            ),
        );
        return {
          kind: "document",
          path: entry?.workspacePath || entry?.path || path,
          name: entry?.name || basenameForPath(path),
          source,
          root,
