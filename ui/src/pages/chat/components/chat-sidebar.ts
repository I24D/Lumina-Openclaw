...
function absoluteFilePath(content: { path: string; root?: string | null }): string | null {
  if (
...
function renderDocumentSidebarContent(
  content: DocumentSidebarContent,
  controls: DocumentViewControls,
) {
  const absolutePath = absoluteFilePath(content);
  return html`
    <section class="sidebar-file-view sidebar-file-view--document">
      <div class="sidebar-file-view__path-bar">
        ...
        <div class="sidebar-file-view__actions">
          ...
          <openclaw-tooltip .content=${t("chat.detailPanel.downloadFile")}>
            <button
              class="btn btn--sm sidebar-file-view__action"
              type="button"
              aria-label=${t("chat.detailPanel.downloadFile")}
              ?disabled=${!controls.downloadReady}
              @click=${controls.onDownload}
            >
              ${icons.download}
            </button>
          </openclaw-tooltip>
      ...
      <div class="document-view">
        ${keyed(controls.mountKey, html`<div class="document-view__mount"></div>`)}
        ${controls.loading
          ? html`
              <div class="document-view__status" role="status">
                <span class="document-view__spinner" aria-hidden="true">${icons.loader}</span>
                <span>${t("chat.detailPanel.documentPreviewLoading")}</span>
              </div>
            `
          : controls.error
            ? html`
                <div class="document-view__status document-view__status--error" role="alert">
                  <span class="document-view__status-title">
                    ${t("chat.detailPanel.documentPreviewFailed")}
                  </span>
                  <span class="document-view__status-detail">${controls.error}</span>
                  <button class="btn btn--sm" type="button" @click=${controls.onRetry}>
                    ${t("common.retry")}
                  </button>
                </div>
              `
...
type MarkdownSidebarProps = {
  ...
  documentView?: DocumentViewControls;
  error: string | null;
      ...
      : content?.kind === "image"
        ...
        : content?.kind === "file"
          ...
          : content?.kind === "document"
            ? content.name.trim() || t("chat.detailPanel.documentPreview")
        ...
        ${props.error
          ...
          : content
            ? content.kind === "file"
              ...
              : content.kind === "document" && props.documentView
                ? renderDocumentSidebarContent(content, props.documentView)
                : content.kind === "session-diff"
...
class ChatDetailPanel extends OpenClawLightDomElement {
  ...
  @property() basePath: string | null = null;
  @property() assistantAttachmentAuthToken: string | null = null;
  ...
  @state() private documentViewerLoading = false;
  @state() private documentViewerError: string | null = null;
  ...
  private fileHash = "";
  private documentOperationVersion = 0;
  private documentViewer: { destroy: () => void; download: () => Promise<void> } | null = null;
  private documentViewerLoad: Promise<void> | null = null;
  private documentViewerAbort: AbortController | null = null;
  ...
  protected override willUpdate(changed: Map<string, unknown>) {
    ...
    this.documentViewerError = null;
    this.documentViewerLoading = this.content?.kind === "document";
    const retainedDraft =
  ...
  protected override updated(changed: Map<string, unknown>) {
    ...
    if (visibleContent?.kind === "document") {
      void this.ensureDocumentViewer();
    }
  ...
  private destroyDocumentViewer() {
    this.documentOperationVersion += 1;
    this.documentViewerAbort?.abort();
    this.documentViewerAbort = null;
    this.documentViewer?.destroy();
    this.documentViewer = null;
    this.documentViewerLoad = null;
  }

  private ensureDocumentViewer(): Promise<void> {
    if (this.documentViewer || this.documentViewerLoad) {
      return this.documentViewerLoad ?? Promise.resolve();
    }
    const content = this.visibleContent;
    const parent = this.querySelector<HTMLElement>(".document-view__mount");
    if (content?.kind !== "document" || !parent) {
      return Promise.resolve();
    }
    const version = this.documentOperationVersion;
    const abortController = new AbortController();
    this.documentViewerAbort = abortController;
    this.documentViewerLoading = true;
    this.documentViewerError = null;
    this.documentViewerLoad = import("./office-file-viewer.runtime.ts")
      .then(({ createOfficeFileViewer }) =>
        createOfficeFileViewer({
          parent,
          source: content.source,
          name: content.name,
          basePath: this.basePath,
          authToken: this.assistantAttachmentAuthToken,
          signal: abortController.signal,
        }),
      )
      .then((viewer) => {
        if (
          version !== this.documentOperationVersion ||
          !this.isConnected ||
      ...
      .catch((error: unknown) => {
        if (version !== this.documentOperationVersion || abortController.signal.aborted) {
          return;
        }
        this.documentViewerError = error instanceof Error ? error.message : String(error);
      })
      .finally(() => {
        if (version === this.documentOperationVersion) {
          this.documentViewerLoad = null;
          this.documentViewerLoading = false;
        }
      });
    return this.documentViewerLoad;
  }

  private readonly downloadDocument = () => {
    void this.documentViewer?.download().catch((error: unknown) => {
      this.documentViewerError = error instanceof Error ? error.message : String(error);
    });
  };

  private readonly retryDocumentViewer = () => {
    this.destroyDocumentViewer();
    this.documentViewerError = null;
    this.documentViewerLoading = true;
  };
  ...
  private readonly openInEditor = (editor: EditorId) => {
    const content = this.visibleContent;
    if (content?.kind !== "file" && content?.kind !== "document") {
      return;
    }
    ...
    this.fileEditorMenuOpen = false;
    openEditor(editor, absPath, content.kind === "file" ? content.line : null);
  };
