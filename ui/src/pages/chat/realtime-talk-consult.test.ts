...
describe("RealtimeTalkSession consult handoff", () => {
  it("submits an isolated Gateway result without waiting for public chat events", async () => {
    const request = vi.fn(async (method: string) => {
      if (method === "talk.client.toolCall") {
        return { result: "The workspace is clean." };
      }
    ...
    expect(submit).toHaveBeenCalledWith("call-1", { result: "The workspace is clean." });
  });
